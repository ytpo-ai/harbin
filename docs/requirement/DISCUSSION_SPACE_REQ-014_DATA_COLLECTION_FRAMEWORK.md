...
- [ ] `ei-data-source.schema.ts` 定义完成（含 config/collectFrequency/scheduleId/statistics 等）
- [ ] `ei-data-record.schema.ts` 定义完成（含 data/rawData/collectedAt/dataCategory/tags 等）
- [ ] 数据源 CRUD API 完整实现（POST/GET/PUT/DELETE + test + collect）
- [ ] 数据记录查询/聚合/删除 API 完整实现
- [ ] 创建数据源时自动创建对应的 `orchestration_schedule`
- [ ] Schedule 执行时能触发 Agent 采集并写入 `ei_data_records`
- [ ] MCP 工具 `data-collection.write-record` 和 `data-collection.get-source-config` 实现
- [ ] 前端 `DataDashboard.tsx` 页面完成（数据源管理 + 数据看板 + 时间筛选）
- [ ] 孵化项目详情页可导航到数据看板
- [ ] 数据源健康度监控（成功率、最近错误）
...
| 文档 | 状态 |
|------|------|
| [开发记录](../development/DISCUSSION_SPACE_REQ-014_DEVELOPMENT.md) | 待创建 |

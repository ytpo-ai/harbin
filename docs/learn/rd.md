...
1. step3（技术方案）、step4（执行开发）的 taskType 必须设为 `development`
2. step5（实现评估）的 taskType 必须设为 `review`
...
- 状态更新时机：
  1. step1 任务生成后，状态更新为 `assigned`（Assigned，需求分配给 Agent）。
  2. step1/step2/step3 生成后，状态更新为 `in_progress`（In Progress）。
  3. step5 生成后，状态更新为 `review`（Review）。

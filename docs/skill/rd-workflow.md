...
- **输入**: phaseInitialize 注入的 taskContext（含需求详情）+ 相关代码/文档
- **输出契约**: 结构化开发计划（含实现步骤、涉及文件/接口清单、测试要点）
- **约束**: 输出中避免引用具体内部工具名称
- **四阶段行为**:
  - **generate**: 生成任务描述，引用 taskContext 中的 requirementId 和需求标题，明确要求执行者输出结构化开发计划
  - **pre_execute**: Planner 通过工具调用 `requirement.update-status` 将需求状态更新为 `in_progress`；检查执行者工具匹配度
...
- **执行角色**: 全栈开发（选择当前状态空闲的具备 `development_exec` 及 `opencode` 能力的 agent）
- **输入**: step1 输出的开发计划
- **输出契约**: 代码 commit 信息（含 commit hash、变更文件列表、变更摘要）
- **约束**: 描述中使用"读取代码"、"修改代码"、"提交变更"等自然语言，禁止引用内部工具名称
- **四阶段行为**:
...
- **四阶段行为**:
  ...
  - **pre_execute**: Planner 通过工具调用 `requirement.update-status` 将需求状态更新为 `review`
...
| 时机 | 状态 | 触发方 | 工具 |
|------|------|--------|------|
| phaseInitialize 完成 | `assigned` | Planner 工具调用 | `requirement.update-status` |
| step1 pre-execute | `in_progress` | Planner 工具调用 | `requirement.update-status` |
| step3 pre-execute | `review` | Planner 工具调用 | `requirement.update-status` |

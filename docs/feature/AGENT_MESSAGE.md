...
| 路径 | 功能 |
|------|------|
| `backend/src/modules/agent-messages/agent-messages.controller.ts` | 直发/发布/ACK/订阅接口 |
| `backend/src/modules/agent-messages/agent-messages.service.ts` | 消息落库、订阅匹配、状态流转、入队 |
| `backend/src/modules/agent-messages/agent-message-dispatcher.service.ts` | Redis 分发消费者（重试/死信） |
| `backend/src/modules/agent-messages/agent-collaboration-automation.service.ts` | 任务事件到拟人协作消息的自动编排 |
| `backend/src/modules/agent-messages/agent-messages.module.ts` | 模块装配 |
| `backend/src/shared/schemas/agent-collaboration-message.schema.ts` | Agent 消息模型 |
| `backend/src/shared/schemas/agent-message-subscription.schema.ts` | 订阅模型 |

...
8. [ ] **子任务手动执行按钮**
   - 子任务不参与 plan run 的自动调度，不会被 step dispatcher 自动推进
   - 子任务创建后在任务列表中显示"执行"按钮，复用 `debug-run` 机制触发单任务执行
   - 执行按钮仅在子任务状态为 `pending` / `assigned` / `failed` 时可用
...
- **子任务与调度器隔离**：子任务不参与 plan run 自动调度，仅通过手动"执行"按钮（debug-run）触发，step dispatcher 推进时需跳过 parentTaskId 非空的任务
...
- 子任务采用手动执行模式（debug-run），不参与 plan run 自动调度，降低与现有调度器的耦合风险

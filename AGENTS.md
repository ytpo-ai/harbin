...
当用户提出具体开发需求时，必须先输出执行计划（plan），并等待用户明确同意后才可开始编码。

### 标准执行顺序
1. 给出计划（plan）
2. 计划得到同意后，将计划更新到 `docs/plan/` 下的 Markdown 文档
3. 完成 plan 文档更新后，再执行开发
4. 开发完成后，先询问用户是否需要沉淀开发总结文档；用户同意后，将文档保留在 `docs/development/` 下，且与 plan 文档同名
...
- 用户明确说明“跳过文档落盘”时，可跳过 `docs/plan/` 或 `docs/development/` 更新

...
`docs/guide/` 作为项目知识的**缓存层**，用于存放对项目结构、模块机制、核心链路的梳理文档，避免每次开发都重复扫描大量代码。

**执行流程**：
1. 接到开发需求后，**优先查阅 `docs/guide/`** 下是否已有与当前需求相关的梳理文档
2. 如果 guide 文档已覆盖所需上下文，直接使用，**不必扫描代码**
3. 如果 guide 文档不存在或信息不足，再进行代码扫描获取上下文
4. 代码扫描完成后，**必须将梳理结果回写到 `docs/guide/`**，供后续 session 复用

**guide 文档规范**：
- 文件命名：使用大写字母 + `&` 或 `_` 分隔的 `.MD` 格式，体现文档主题（如 `AGENT&ROLE.MD`、`SKILL_LOAD.md`）
- 内容范围：模块职责、核心链路、关键 Schema/DTO、调用关系、配置要点等**不易频繁变动的结构性知识**
- 不适合放入 guide 的内容：临时调试记录、单次 fix 细节、频繁变动的业务逻辑快照
- 如果发现已有 guide 文档内容过时，应在本次开发中**顺带更新**，保持缓存有效性

**与功能文档的关系**：
- `docs/guide/`：偏向**技术实现梳理**（代码结构、模块机制、链路分析）
- `docs/feature/`：偏向**功能维度描述**（功能说明、使用方式、配置项）
- 两者互补，guide 侧重"代码怎么跑的"，feature 侧重"功能是什么"

### 开发前必须阅读功能文档

**规则**：
1. 接到开发需求后，先查阅 `docs/guide/` 获取技术上下文（见上述缓存协议）

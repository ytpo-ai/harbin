    ...
    <div className="w-14 shrink-0 border-r border-slate-200 px-2 py-3">
      <div className="space-y-1">
        {rounds.map((item, index) => {
          ...
          return (
            <div key={item.round} className="relative pb-2 last:pb-0">
              {index < rounds.length - 1 && <span className="absolute left-[5px] top-3 h-[18px] w-px bg-slate-200" aria-hidden="true" />}
              <button className="flex w-full items-center gap-1 text-left" onClick={() => onSelectRound(item.round)}>

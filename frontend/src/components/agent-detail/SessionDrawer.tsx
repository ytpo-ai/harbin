...
export const SessionDrawer: React.FC<SessionDrawerProps> = ({ state }) => {
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-slate-200/60 px-6 py-5">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">查看上下文与消息轨迹</p>
            </div>
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {state.sessionDetailQuery.data?.id ? (
                      <button
                        onClick={() => state.copyText(state.sessionDetailQuery.data?.id || '')}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-slate-200/60 bg-slate-50/50 px-4 py-3">
                    ...
                    <p className="mt-1.5 truncate font-mono text-xs text-slate-700">{state.sessionDetailQuery.data?.id}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200/60 bg-slate-50/50 px-4 py-3">
                    <p className="text-xs font-medium uppercase tracking-wider text-slate-400">状态</p>
                    <p className="mt-1.5 text-sm font-medium text-slate-700">{state.sessionDetailQuery.data?.status}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200/60 bg-slate-50/50 px-4 py-3">
                    <p className="text-xs font-medium uppercase tracking-wider text-slate-400">类型</p>
                    <p className="mt-1.5 text-sm font-medium text-slate-700">{state.sessionDetailQuery.data?.sessionType}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200/60 bg-slate-50/50 px-4 py-3">
                    <p className="text-xs font-medium uppercase tracking-wider text-slate-400">最近活跃</p>
                    <p className="mt-1.5 text-sm text-slate-600">
                      {state.sessionDetailQuery.data?.lastActiveAt ? new Date(state.sessionDetailQuery.data.lastActiveAt).toLocaleString() : '-'}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-slate-200/60 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-xs md:grid-cols-4">
                    <div className="rounded-lg bg-slate-50 p-2.5">
                      ...
                      <p className="mt-1 truncate font-mono text-slate-600">{state.sessionDetailQuery.data?.planContext?.linkedPlanId || '-'}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-2.5">
                      <p className="text-slate-400">Task</p>
                      <p className="mt-1 truncate font-mono text-slate-600">{state.sessionDetailQuery.data?.planContext?.linkedTaskId || '-'}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-2.5">
                      <p className="text-slate-400">Meeting</p>
                      <p className="mt-1 truncate font-mono text-slate-600">{state.sessionDetailQuery.data?.meetingContext?.meetingId || '-'}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-2.5">
                      <p className="text-slate-400">Agenda</p>
                      <p className="mt-1 truncate font-mono text-slate-600">{state.sessionDetailQuery.data?.meetingContext?.agendaId || '-'}</p>
                    </div>
                  </div>
                </div>

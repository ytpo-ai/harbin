    ...
    >
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs font-semibold tracking-wide text-slate-500 shrink-0">#{task.order + 1}</span>
          <span className="text-sm font-medium text-slate-900 truncate">{task.title || '未命名任务'}</span>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className={`text-xs px-1.5 py-0.5 rounded ${STATUS_COLOR[task.status] || STATUS_COLOR.pending}`}>
            {task.status}
          </span>
          <span className="text-xs text-slate-400">{formatExecutor(task, agentNameById)}</span>
        </div>
      </button>

...
export class PlannerService {
  ...
  private buildIncrementalPlannerPrompt(context: IncrementalPlannerContext): string {
    ...
    sections.push('- taskType 用于指定任务执行类型。大多数任务使用 general；仅当任务确实需要信息检索/调研时才用 research；开发域任务使用 development.plan/development.exec/development.review。');
    sections.push('');
  ...
  private normalizeTaskType(input: unknown):
    | 'research'
    | 'development.plan'
    | 'development.exec'
    | 'development.review'

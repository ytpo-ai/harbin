...
const DEFAULT_PLANNER_GENERATING_PROMPT = [
  ...
  '- 允许调用 `builtin.sys-mg.mcp.orchestration.submit-task` 和 `builtin.sys-mg.mcp.tool-schema.get`。',
...
const DEFAULT_PLANNER_INITIALIZE_PROMPT = [
  ...
  '',
  '**步骤**：',
  '1. 调用 `builtin.sys-mg.mcp.agent.list` 获取可用 agent 列表（含 capabilitySet）。',
  ...
  '    {',
  '      "tool": string,       // 工具 ID（如 builtin.engineering.mcp.requirement.update-status）',
  ...
  '- 如果不确定工具参数格式，可调用 `builtin.sys-mg.mcp.tool-schema.get`。',
...
const DEFAULT_POST_EXECUTE_CONTEXT_PROMPT = [
  ...
  '- 可调用 get-tool-schema 查询 submit-task-run-result 参数格式。',
  '- 你只需分析执行结果，返回决策 JSON。',
  '',
  '请进行执行后决策，返回 JSON。',
  '',

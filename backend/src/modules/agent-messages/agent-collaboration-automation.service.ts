...
import { AgentMessagesService, ORCHESTRATION_TASK_EVENT_HOOK_CHANNEL } from './agent-messages.service';
...
@Injectable()
export class AgentCollaborationAutomationService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(AgentCollaborationAutomationService.name);
  ...
  private async handleTaskCreated(event: TaskEventPayload): Promise<void> {
    ...
    const direct = await this.agentMessagesService.sendDirectMessage({
      ...
      source: 'agent-collaboration-automation',
      dedupKey: `auto:cto-assign:${event.taskId}:${executorAgentId}`,
    });

    await this.agentMessagesService.acknowledgeMessage(direct.messageId, executorAgentId, 'delivered').catch(() => undefined);

    await this.agentMessagesService.sendDirectMessage({

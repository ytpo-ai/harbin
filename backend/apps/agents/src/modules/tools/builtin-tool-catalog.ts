...
export const BUILTIN_TOOLS = [
      ...
      {
        id: TOOL_ID__DATA_COLLECTION_GET_SOURCE_CONFIG,
        name: 'Data Collection Get Source Config',
        description: 'Get EI data source configuration by sourceId',
        type: 'api_call' as const,
        category: 'Engineering Intelligence',
      ...
      {
        id: TOOL_ID__DATA_COLLECTION_WRITE_RECORD,
        name: 'Data Collection Write Record',
        description: 'Write EI data collection record by sourceId',
        type: 'api_call' as const,
        category: 'Engineering Intelligence',
      ...
      {
        id: TOOL_ID__DISCUSSION_OUTLINE_MANAGE,
        name: 'Discussion Outline Manage',
        description: 'Manage discussion outline CRUD and section enrichment via a unified MCP action',
        prompt:
          '当你需要维护讨论大纲时，统一调用 discussion-outline.manage。action 支持 get_outline/create_section/update_section/delete_section/reorder_section/enrich_section/clear_enrichments。enrich_section 必须关注 acceptedEntries 或 fallbackReason，不要把 blocked/澄清回执当成业务结果。',
        type: 'api_call' as const,
        category: 'Engineering Intelligence',
        requiredPermissions: [{ id: 'discussion_outline_write', name: 'Discussion Outline Write', level: 'intermediate' }],
        tokenCost: 4,
        implementation: {
          type: 'built_in' as const,
          parameters: {
            ...
            properties: {
              ...
              order: { type: 'number' },
              status: { type: 'string', enum: ['draft', 'enriching', 'sufficient', 'review'] },

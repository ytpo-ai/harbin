...
export class EngineeringIntelligenceService {
  ...
  async listRequirements(query: ListRequirementsDto) {
  ...
  async getRequirementBoard() {
    const requirements = await this.requirementModel.find({}).sort({ updatedAt: -1 }).lean().exec();
  ...
  async syncRequirementToGithub(requirementId: string, payload: SyncRequirementToGithubDto) {
    ...
    const response = await fetch(`${this.githubApiBase}/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/issues`, {
      method: 'POST',
      headers: {
        Accept: 'application/vnd.github+json',
        Authorization: `Bearer ${this.getGitHubToken()}`,
    ...
    if (!response.ok) {
      ...
      requirement.githubLink = {
        ...
        syncedAt: new Date(),
        lastError: `GitHub API error (${response.status}): ${bodyText}`,
      };
      await requirement.save();
      throw new BadRequestException(`GitHub API error (${response.status}): ${bodyText}`);
    }

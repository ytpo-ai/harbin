...
const TOOL_PROMPT_PRESETS: Record<string, string> = {
  ...
  'builtin.engineering.internal.fs.docs_read':
    '当用户询问"当前系统实现了哪些核心功能/系统能力清单/docs里实现了什么"时，优先级如下：1) 优先使用 builtin.engineering.internal.fs.repo_read 执行 "git log"、"ls docs/"、"cat docs/..."、"grep ..." 等命令自行读取；2) 其次调用 builtin.engineering.internal.fs.docs_read 读取文档。若 builtin.engineering.internal.fs.docs_read 返回 0 命中或 fallback 信号，必须自动重试（放宽 focus 或不传 focus），仍失败再切换 builtin.engineering.internal.fs.repo_read 直接列目录并读取文档；不要向用户发起二选一确认。必须基于实际读取的内容回答，不得臆测。',
  'builtin.engineering.internal.git.commit_list':
    '当用户询问"最近24小时/最近一天系统主要更新"时，优先级如下：1) 优先使用 builtin.engineering.internal.fs.repo_read 执行 "git log --since=..." 等命令自行读取提交记录；2) 其次调用 builtin.engineering.internal.git.commit_list。必须基于实际提交内容回答，不得臆测。',
  'builtin.engineering.internal.fs.repo_read':
    '你拥有 builtin.engineering.internal.fs.repo_read 工具，可执行只读 bash 命令（如 git log、cat、ls、grep 等）来读取本地仓库文件。当你需要了解代码或文档内容时，请优先使用 builtin.engineering.internal.fs.repo_read 直接读取。',
  'builtin.engineering.internal.fs.docs.write':
    '当你需要新增或更新研发文档时，调用 builtin.engineering.internal.fs.docs.write；仅写 docs/** 下的 .md 文件，优先使用 create/update/append 明确意图，避免覆盖不相关内容。',
